#ifndef CLASSEMENT_MANCHE_H
#define CLASSEMENT_MANCHE_H
...
#include <QVector>
#include <chrono>
class Classement_manche
{
public:
    Classement_manche();
    // Constructor
    Classement_manche(int idConcurrent, int idManche, QString nomManche, int numConcurrent, std::chrono::milliseconds totalChrono,int id_categorie,QString nom_categorie,int points);
    ...
    QVector<Classement_manche> fetchData(QSqlQuery qry);
    void displayClassement(QTableWidget* tableWidget,const QVector<Classement_manche>& classe,const MainWindow& mw);
